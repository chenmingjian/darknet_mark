...
{
    *net->seen += net->batch;//seen网络看过的数据的数量。
    net->train = 1; //疑似训练标记？
    forward_network(net);
    backward_network(net);
    float error = *net->cost;
    if(((*net->seen)/net->batch)%net->subdivisions == 0) update_network(net);//subdivisions:每几个batch一更新网络。
    return error;//如果不到subdivision个batch的话只返回err不更新参数。
}
