...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    for(i = 0; i < ngpus; ++i){
        ...
        nets[i]->learning_rate *= ngpus;//在多GPU的情况下，learning_rate为什么需要变大？多GPU的计算情况是怎样的？
    }
    srand(time(0));
    network *net = nets[0];

    int imgs = net->batch * net->subdivisions * ngpus;//batch * subdivision 意味着什么？   -->subdivisions 在这份代码中表示同时装入内存的图片数要分成几个batch，既同时装入内存的batch数是batch*subdivisions。
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        if(i%100==0){//每100个batch backup 一次。
#ifdef GPU
